...
def generate_projectresult_export_csv(csv_download_pk):
    logger.info("Creating CSVDownload(pk={}) - project result export".format(csv_download_pk))
    save_csv(csv_download_pk, projectresult_export())


@shared_task
def generate_diagnostic_export_csv(csv_download_pk):
    logger.info("Creating CSVDownload(pk={}) - diagnostic export".format(csv_download_pk))
